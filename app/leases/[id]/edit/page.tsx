  ...
  return (
    <AppShell>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderBottom: '0.5px solid rgba(255,255,255,0.07)', background: '#161614', flexShrink: 0 }}>
        <div>
          <a href={'/leases/' + params.id} style={{ fontSize: '11px', color: '#5A5A56', textDecoration: 'none' }}>← Back to Lease</a>
          <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '16px', fontWeight: 700, color: '#F0EEE8', marginTop: '2px' }}>Edit Lease</div>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <a href={'/leases/' + params.id} style={btnG}>Cancel</a>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        ...
        <div style={card}>
          ...
          <div style={{ ...g2, marginBottom: '12px' }}>
            <div><label style={lbl}>Monthly Rent *</label><input style={inp} type='number' value={form.rent_amount} onChange={e => set('rent_amount', e.target.value)} /></div>
            <div><label style={lbl}>Security Deposit</label><input style={inp} type='number' value={form.security_deposit} onChange={e => set('security_deposit', e.target.value)} /></div>
          </div>
          <div style={{ ...g3, marginBottom: '12px' }}>
            <div><label style={lbl}>Due Day</label><input style={inp} type='number' min='1' max='28' value={form.due_day} onChange={e => set('due_day', e.target.value)} /></div>
            <div><label style={lbl}>Grace Period (days)</label><input style={inp} type='number' value={form.grace_period_days} onChange={e => set('grace_period_days', e.target.value)} /></div>
            <div><label style={lbl}>Late Fee</label><input style={inp} type='number' value={form.late_fee_amount} onChange={e => set('late_fee_amount', e.target.value)} /></div>
          </div>
          <div style={g2}>
            ...
            <div><label style={lbl}>Pet Deposit</label><input style={inp} type='number' value={form.pet_deposit} onChange={e => set('pet_deposit', e.target.value)} /></div>
          </div>
        </div>
        <div style={card}>
          ...
          <div style={{ ...g2, marginBottom: '12px' }}>
            ...
            <div><label style={lbl}>Parking Spaces</label><input style={inp} type='number' value={form.parking_spaces} onChange={e => set('parking_spaces', e.target.value)} /></div>
          </div>
          <div><label style={lbl}>Special Clauses</label><textarea style={{ ...inp, resize: 'vertical' }} rows={3} value={form.special_clauses} onChange={e => set('special_clauses', e.target.value)} /></div>
        </div>
