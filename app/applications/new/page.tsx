  ...
  return (
    <AppShell>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderBottom: '0.5px solid rgba(255,255,255,0.07)', background: '#161614', flexShrink: 0 }}>
        <div>
          <a href='/applications' style={{ fontSize: '11px', color: '#5A5A56', textDecoration: 'none' }}>← Applications</a>
          <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '16px', fontWeight: 700, color: '#F0EEE8', marginTop: '2px' }}>New Application</div>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <a href='/applications' style={btnG}>Cancel</a>
          <button style={btnP} onClick={save} disabled={saving}>{saving ? 'Saving...' : '+ Submit Application'}</button>
        </div>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        ...
        <div style={card}>
          ...
          <div style={{ marginBottom: '12px' }}><label style={lbl}>Full Name *</label><input style={inp} value={form.applicant_name} onChange={e => set('applicant_name', e.target.value)} /></div>
          ...
          <div style={g2}>
            <div><label style={lbl}>Desired Move In</label><input style={inp} type='date' value={form.desired_move_in} onChange={e => set('desired_move_in', e.target.value)} /></div>
            <div><label style={lbl}>Monthly Income</label><input style={inp} type='number' placeholder='0.00' value={form.monthly_income} onChange={e => set('monthly_income', e.target.value)} /></div>
          </div>
        </div>
        <div style={card}>
          <div style={secTtl}>Employment</div>
          <div><label style={lbl}>Employer Name</label><input style={inp} value={form.employer_name} onChange={e => set('employer_name', e.target.value)} /></div>
        </div>
        <div style={card}>
          <div style={secTtl}>Rental History</div>
          <div style={{ ...g2, marginBottom: '12px' }}>
            <div><label style={lbl}>Previous Landlord</label><input style={inp} value={form.previous_landlord_name} onChange={e => set('previous_landlord_name', e.target.value)} /></div>
            <div><label style={lbl}>Landlord Phone</label><input style={inp} value={form.previous_landlord_phone} onChange={e => set('previous_landlord_phone', e.target.value)} /></div>
          </div>
          <div style={{ ...g2, marginBottom: '12px' }}>
            <div><label style={lbl}>Previous Rent</label><input style={inp} type='number' value={form.previous_rent} onChange={e => set('previous_rent', e.target.value)} /></div>
            <div><label style={lbl}>Reason for Moving</label><input style={inp} value={form.reason_for_moving} onChange={e => set('reason_for_moving', e.target.value)} /></div>
          </div>
          <div style={{ ...g2 }}>
            <div><label style={lbl}>Pets</label>
              <select style={inp} value={form.has_pets ? 'true' : 'false'} onChange={e => set('has_pets', e.target.value === 'true')}>
                <option value='false'>No Pets</option>
                <option value='true'>Has Pets</option>
              </select>
            </div>
            <div><label style={lbl}>Eviction History</label>
              <select style={inp} value={form.has_eviction_history ? 'true' : 'false'} onChange={e => set('has_eviction_history', e.target.value === 'true')}>
